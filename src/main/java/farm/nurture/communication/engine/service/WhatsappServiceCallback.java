...
import farm.nurture.communication.engine.dto.KarixWhatsAppResponse;
...
@Slf4j
public class WhatsappServiceCallback implements FutureCallback<HttpResponse> {
    ...
    public WhatsappServiceCallback(WhatsappService whatsappService, Producer producer, DerivedCommunicationEvent event, MessageAcknowledgementRepository messageAcknowledgementRepository, String referenceId, LanguageCache languageCache) {
    ...
    private void processError(HttpEntity entity, int statusCode) throws IOException {
        IMetricCounter counter = metrics.getIMetricCounter(MetricGroupNames.NF_CE_WHATSAPP_SERVICE, "api_exception", "status_code");
        ...
        log.error("Got error in sending Whatsapp message for event : {}, status code: {}, responseBody: {}", event, statusCode, responseBody);
    ...
    private void retryEvent() {
        ...
        if (event.getRetryCount() < maxRetries) {
            ...
            this.whatsappService.sendMessage(event);
        } else {
        ...
        try {
            ...
            if (BaseHttpResponseException.Family.familyOf(statusCode) == BaseHttpResponseException.Family.SUCCESSFUL) {
                ...
                if(event.getVendor() == VendorType.GUPSHUP) {
                    ...
                    KarixWhatsAppResponse karixWhatsAppResponse = Serializer.DEFAULT_JSON_SERIALIZER.deserialize(result, KarixWhatsAppResponse.class);
                    if (!karixWhatsAppResponse.getStatusCode().equals("200")) {
                        processErrorResponse(result);
                    }else {
                        String vendorMessageId = karixWhatsAppResponse.getMid();
                        processSuccessResponse(vendorMessageId, result);
                    }

                }
            } else {
                log.error("Error in sending Whatsapp message for event : {}", event);
                this.processError(entity, statusCode);
            }

        } catch (Exception e) {
            log.error("Exception in processing WhatsappServiceCallback for event : {}, error : {}", event, e.getMessage(), e);
        }
    }

    private void processSuccessResponse(String vendorMessageId, String result) {

        messageAcknowledgementRepository.updateMessageAcknowledgementVenodrDelivery(MessageAcknowledgement.State.VENDOR_DELIVERED, Timestamp.valueOf(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())), vendorMessageId, referenceId, null, CommunicationChannel.WHATSAPP.toString());
        log.info("Successfully delivered Whatsapp message for event : {}, response : {}", event, result);
        metrics.onIncrement(MetricGroupNames.NF_CE_WHATSAPP_SERVICE, "api_successful", Constants.LABEL_TEMPLATE_LANGUAGE_STATE_LIST, event.getTemplateName(),
    ...
    private void processErrorResponse(String result) {
        ...
        metrics.onIncrement(MetricGroupNames.NF_CE_WHATSAPP_SERVICE, "api_error_response", Constants.LABEL_TEMPLATE_LANGUAGE_STATE_LIST, event.getTemplateName(), languageCache.getLanguageById(event.getLanguageId()).getName(),
                MessageAcknowledgement.State.VENDOR_DELIVERED.name(), event.getVendor().name());
        messageAcknowledgementRepository.updateMessageAcknowledgementVenodrDelivery(MessageAcknowledgement.State.VENDOR_UNDELIVERED, null, "", referenceId, attributes, CommunicationChannel.WHATSAPP.toString());
        logError(result);
        retryEvent();
    }

    private void logError(String result) {
        log.error("Error in sending Whatsapp message for event : {}, error : {}", event, result);
        //Add logic for error handlng
    }

    @Override
    public void failed(Exception e) {
        CommunicationEventHandler.IN_FLIGHT_COUNTER.decrementAndGet();
        log.error("Whatsapp service call failed for event : {}, Error : {}", event, e.getMessage(), e);
        metrics.onIncrement(MetricGroupNames.NF_CE_WHATSAPP_SERVICE, "api_failed");
        retryEvent();
    }
    ...
    public void cancelled() {
        CommunicationEventHandler.IN_FLIGHT_COUNTER.decrementAndGet();
        log.error("Whatsapp service call cancelled for event : {}", event);
        metrics.onIncrement(MetricGroupNames.NF_CE_WHATSAPP_SERVICE, "api_cancelled");
    }
}
